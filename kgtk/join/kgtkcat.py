...
class KgtkCat:
    ...
    output_column_names: typing.Optional[typing.List[str]] = attr.ib(
    ...
    old_column_names: typing.Optional[typing.List[str]] = attr.ib(
        validator=attr.validators.optional(
            attr.validators.deep_iterable(
                member_validator=attr.validators.instance_of(str),
                iterable_validator=attr.validators.instance_of(list),
    ...
    new_column_names: typing.Optional[typing.List[str]] = attr.ib(
        validator=attr.validators.optional(
            attr.validators.deep_iterable(
                member_validator=attr.validators.instance_of(str),
                iterable_validator=attr.validators.instance_of(list),
    ...
    output_format: typing.Optional[str] = attr.ib(
    ...
    def process(self):
        ...
        if self.verbose:
            print(
                "Opening the %d input files." % len(self.input_file_paths),
        ...
        for idx, input_file_path in enumerate(self.input_file_paths):
            if str(input_file_path) == "-":
                ...
                if self.verbose:
                    print(
                        "Opening file %d: standard input" % idx + 1,
                        file=self.error_file,
                        flush=True,
                    )
            else:
                if self.verbose:
                    print(
                        "Opening file %d: %s" % (idx + 1, str(input_file_path)),
                        file=self.error_file,
                        flush=True,
                    )
            ...
            if kr.is_edge_file:
                if is_node_file:
                    raise ValueError(
                        "Cannot merge an edge file to a node file: %s" % input_file_path
                    )
                ...
                if is_edge_file:
                    raise ValueError(
                        "Cannot merge a node file to an edge file: %s" % input_file_path
                    )
                if is_node_file == False and self.verbose:
                    print(
                        "The output file will be an node file.",
                        file=self.error_file,
                        flush=True,
                    )
                is_node_file = True
            ...
            if self.verbose or self.very_verbose:
                print(
                    "Mapping the %d column names in %s."
        ...
        if self.verbose or self.very_verbose:
            print(
                "There are %d merged columns." % len(kmc.column_names),
                file=self.error_file,
                flush=True,
            )
        if self.very_verbose:
        ...
        if self.output_column_names is not None:
            if self.verbose:
                print(
                    "There are %d new output column names."
        ...
        if is_edge_file:
            ...
            if self.verbose:
                print(
                    "Opening the output node file: %s" % str(self.output_path),
                    file=self.error_file,
                    flush=True,
                )
        else:
            if self.verbose:
                print(
                    "Opening the output file: %s" % str(self.output_path),
                    file=self.error_file,
                    flush=True,
                )
        ...
        for idx, kr in enumerate(krs):
            ...
            if self.verbose:
                print(
                    "Copying data from file %d: %s" % (idx + 1, input_file_path),
                    file=self.error_file,
                    flush=True,
                )
            ...
            if self.verbose:
                print(
                    "Read %d data lines from file %d: %s"
                    % (input_data_lines, idx + 1, input_file_path)
                )

        if self.verbose:
            print(
                "Wrote %d lines total from %d files" % (output_data_lines, len(krs)),
                file=self.error_file,
                flush=True,
            )
...
def main():
    ...
    if args.show_options:
        ...
        if args.output_format is not None:
            print(
                "--output-format=%s" % args.output_format, file=error_file, flush=True
            )
        if args.output_column_names is not None:
            print(
                "--output-columns=%s" % " ".join(args.output_column_names),
                file=error_file,
                flush=True,
            )
        if args.old_column_names is not None:
            print(
                "--old-columns=%s" % " ".join(args.old_column_names),
                file=error_file,
                flush=True,
            )
        if args.new_column_names is not None:
            print(
                "--new-columns=%s" % " ".join(args.new_column_names),
                file=error_file,
                flush=True,
            )
