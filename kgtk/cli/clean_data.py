...
def add_arguments_extended(parser: KGTKArgumentParser, parsed_shared_args: Namespace):
    ...
    parser.add_argument(
        "input_file",
        nargs="?",
        help="The KGTK file to read.  May be omitted or '-' for stdin.",
        type=Path,
    )
    parser.add_argument(
        "output_file",
        nargs="?",
        help="The KGTK file to write.  May be omitted or '-' for stdout.",
        type=Path,
    )
...
def run(
    input_file: typing.Optional[Path],
    ...
    **kwargs  # Whatever KgtkReaderOptions and KgtkValueOptions want.
    ...
    if show_options:
        print(
            "input: %s" % (str(input_file) if input_file is not None else "-"),
            file=error_file,
        )
        print(
            "output: %s" % (str(output_file) if output_file is not None else "-"),
            file=error_file,
        )
    ...
    if verbose:
        if input_file is not None:
            print(
                "Cleaning data from '%s'" % str(input_file), file=error_file, flush=True
            )
        else:
            print("Cleaning data from stdin", file=error_file, flush=True)
        if output_file is not None:
            print(
                "Writing data to '%s'" % str(output_file), file=error_file, flush=True
            )
        else:
    ...
    try:
        ...
        kw: KgtkWriter = KgtkWriter.open(
            kr.column_names, output_file, verbose=verbose, very_verbose=very_verbose
        )
        ...
        if verbose:
            print(
                "Copied %d clean data lines" % line_count, file=error_file, flush=True
            )
        return 0
