...
class KgtkJoiner(KgtkFormat):
    ...
    output_path: typing.Optional[Path] = attr.ib(
    ...
    left_join: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    right_join: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )

    # The fllowing may be specified only when both input files are edge files:
    join_on_label: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    join_on_node2: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    ...
    prefix: typing.Optional[str] = attr.ib(
    ...
    field_separator: str = attr.ib(
        validator=attr.validators.instance_of(str), default=KgtkFormat.LIST_SEPARATOR
    )
    ...
    def node1_column_idx(self, kr: KgtkReader, who: str) -> int:
        ...
        if idx < 0:
            ...
            raise ValueError(
                "KgtkJoiner: unknown node1 column index in KGTK %s edge file." % who
            )
        return idx
    ...
    def id_column_idx(self, kr: KgtkReader, who: str) -> int:
        ...
        if idx < 0:
            # TODO: throw a better exception
            raise ValueError(
                "KgtkJoiner: unknown id column index in KGTK %s node file." % who
            )
        return idx

    def build_join_key(
        self, kr: KgtkReader, join_idx_list: typing.List[int], row: typing.List[str]
    ) -> str:
        key: str = ""
        join_idx: int
        first: bool = True
    ...
    def multi_column_key_set(
        self, kr: KgtkReader, join_idx_list: typing.List[int]
    ) -> typing.Set[str]:
    ...
    def build_join_idx_list(
        self, kr: KgtkReader, who: str, join_columns: typing.Optional[typing.List[str]]
    ) -> typing.List[int]:
        join_idx: int
        ...
        if join_columns is not None and len(join_columns) > 0:
            if self.verbose:
                print(
                    "Using %s file join columns: %s" % (who, " ".join(join_columns)),
            ...
            for join_column in join_columns:
                if join_column not in kr.column_name_map:
                    raise ValueError(
                        "Join column %s not found in in the %s input file"
                        % (join_column, who)
                    )
                join_idx = kr.column_name_map[join_column]
                if self.verbose:
                    print(
                        "Join column %d: %s (index %d in the %s input file)"
                        % (col_num, join_column, join_idx, who),
                        file=self.error_file,
                        flush=True,
                    )
        ...
        if kr.is_edge_file:
            ...
            if self.verbose:
                print(
                    "Joining on node1 (index %s in the %s input file)"
                    % (join_idx, who),
                    file=self.error_file,
                    flush=True,
                )
            ...
            if self.verbose:
                print(
                    "Joining on id (index %s in the %s input file)" % (join_idx, who),
                    file=self.error_file,
                    flush=True,
                )
            join_idx_list.append(join_idx)
            ...
            raise ValueError(
                "Quasi-KGTK files require an explicit list of join columns"
            )
        ...
        if self.join_on_label or self.join_on_node2:
            if self.join_on_label:
                if kr.label_column_idx < 0:
                    raise ValueError(
                        "join_on_label may not be used because the %s input file does not have a label column."
                        % who
                    )
                if self.verbose:
                    print(
                        "Joining on label (index %s in the %s input file)"
                        % (kr.label_column_idx, who),
                        file=self.error_file,
                        flush=True,
                    )
            ...
            if self.join_on_node2:
                if kr.node2_column_idx < 0:
                    raise ValueError(
                        "join_on_node2 may not be used because the %s input file does not have a node2 column."
                        % who
                    )
                if self.verbose:
                    print(
                        "Joining on node2 (index %s in the %s input file)"
                        % (kr.node2_column_idx, who),
                        file=self.error_file,
                        flush=True,
                    )
    ...
    def extract_join_key_set(
        self, file_path: Path, who: str, join_idx_list: typing.List[int]
    ) -> typing.Set[str]:
        if self.verbose:
            print(
                "Extracting the join key set from the %s input file: %s"
                % (who, str(file_path)),
                file=self.error_file,
                flush=True,
            )
    ...
    def join_key_sets(
        self,
        left_join_idx_list: typing.List[int],
        right_join_idx_list: typing.List[int],
        ...
        if self.left_join and self.right_join:
            if self.verbose:
                print(
                    "Outer join, no need to compute join keys.",
                    file=self.error_file,
                    flush=True,
                )
            return None
            ...
            if self.verbose:
                print(
                    "Computing the left join key set", file=self.error_file, flush=True
                )
            join_key_set = self.extract_join_key_set(
                self.left_file_path, self.LEFT, left_join_idx_list
            ).copy()
            if self.verbose:
                print(
                    "There are %d keys in the left join key set." % len(join_key_set),
                    file=self.error_file,
                    flush=True,
                )
            return join_key_set
            ...
            if self.verbose:
                print(
                    "Computing the right join key set", file=self.error_file, flush=True
                )
            join_key_set = self.extract_join_key_set(
                self.right_file_path, self.RIGHT, right_join_idx_list
            ).copy()
            if self.verbose:
                print(
                    "There are %d keys in the right join key set." % len(join_key_set),
                    file=self.error_file,
                    flush=True,
                )
            return join_key_set
            ...
            if self.verbose:
                print(
                    "Computing the inner join key set", file=self.error_file, flush=True
                )
            left_join_key_set: typing.Set[str] = self.extract_join_key_set(
                self.left_file_path, self.LEFT, left_join_idx_list
            )
            if self.verbose:
                print(
                    "There are %d keys in the left file key set."
                    % len(left_join_key_set),
                    file=self.error_file,
                    flush=True,
                )
            ...
            if self.verbose:
                print(
                    "There are %d keys in the right file key set."
                    % len(right_join_key_set),
                    file=self.error_file,
                    flush=True,
                )
            ...
            if self.verbose:
                print(
                    "There are %d keys in the inner join key set." % len(join_key_set),
                    file=self.error_file,
                    flush=True,
                )
            return join_key_set
    ...
    def ok_to_join(self, left_kr: KgtkReader, right_kr: KgtkReader) -> bool:
        if left_kr.is_edge_file and right_kr.is_edge_file:
            if self.verbose:
                print(
                    "Both input files are edge files.", file=self.error_file, flush=True
                )
            return True
            ...
            if self.verbose:
                print(
                    "Both input files are node files.", file=self.error_file, flush=True
                )
            return True

        elif (not (left_kr.is_node_file or left_kr.is_edge_file)) or (
            not (right_kr.is_edge_file or right_kr.is_node_file)
        ):
            if self.verbose:
                print(
                    "One or both input files are quasi-KGTK files.",
                    file=self.error_file,
                    flush=True,
                )
            return True
    ...
    def process(self):
        if self.verbose:
            print(
                "Opening the left edge file: %s" % str(self.left_file_path),
                file=self.error_file,
                flush=True,
            )
        left_kr: KgtkReader = KgtkReader.open(
            self.left_file_path,
            who="left input",
        ...
        if self.verbose:
            print(
                "Opening the right edge file: %s" % str(self.right_file_path),
                file=self.error_file,
                flush=True,
            )
        right_kr: KgtkReader = KgtkReader.open(
        ...
        left_join_idx_list: typing.List[int] = self.build_join_idx_list(
            left_kr, self.LEFT, self.left_join_columns
        )
        right_join_idx_list: typing.List[int] = self.build_join_idx_list(
            right_kr, self.RIGHT, self.right_join_columns
        )
        if len(left_join_idx_list) != len(right_join_idx_list):
            print(
                "the left join key has %d components, the right join key has %d columns. Exiting."
                % (len(left_join_idx_list), len(right_join_idx_list)),
                file=self.error_file,
                flush=True,
            )
            left_kr.close()
        ...
        joined_key_set: typing.Optional[typing.Set[str]] = self.join_key_sets(
            left_join_idx_list, right_join_idx_list
        )

        if self.verbose:
            print(
                "Mapping the column names for the join.",
                file=self.error_file,
                flush=True,
            )
        kmc: KgtkMergeColumns = KgtkMergeColumns()
        kmc.merge(left_kr.column_names)
        right_column_names: typing.List[str] = kmc.merge(
            right_kr.column_names, prefix=self.prefix
        )
        joined_column_names: typing.List[str] = kmc.column_names

        if self.verbose:
            print(
                "       left   columns: %s" % " ".join(left_kr.column_names),
                file=self.error_file,
                flush=True,
            )
            print(
                "       right  columns: %s" % " ".join(right_kr.column_names),
                file=self.error_file,
                flush=True,
            )
            print(
                "mapped right  columns: %s" % " ".join(right_column_names),
                file=self.error_file,
                flush=True,
            )
            print(
                "       joined columns: %s" % " ".join(joined_column_names),
                file=self.error_file,
                flush=True,
            )
        ...
        if self.verbose:
            print(
                "Opening the output edge file: %s" % str(self.output_path),
                file=self.error_file,
                flush=True,
            )
        ...
        if self.verbose:
            print(
                "Processing the left input file: %s" % str(self.left_file_path),
                file=self.error_file,
                flush=True,
            )
        row: typing.List[str]
        ...
        if self.verbose:
            print(
                "Processing the right input file: %s" % str(self.right_file_path),
                file=self.error_file,
                flush=True,
            )
        ...
        if self.verbose:
            ...
            print(
                "%d left input data lines read, %d kept"
                % (left_data_lines_read, left_data_lines_kept),
                file=self.error_file,
                flush=True,
            )
            print(
                "%d right input data lines read, %d kept"
                % (right_data_lines_read, right_data_lines_kept),
                file=self.error_file,
                flush=True,
            )
            print(
                "%d data lines written." % output_data_lines,
                file=self.error_file,
                flush=True,
            )
...
def main():
    ...
    parser.add_argument(
        dest="left_file_path", help="The left KGTK file to join", type=Path
    )
    parser.add_argument(
        dest="right_file_path", help="The right KGTK file to join", type=Path
    )
    parser.add_argument(
        "--field-separator",
        dest="field_separator",
        help="Separator for multifield keys",
    ...
    parser.add_argument(
        "--left-file-join-columns",
        dest="left_join_columns",
        help="Left file join columns.",
        nargs="+",
    )
    ...
    parser.add_argument(
        "--left-join",
        dest="left_join",
        help="Perform a left outer join (default=%(default)s).",
        type=optional_bool,
        nargs="?",
        const=True,
        default=False,
    ...
    parser.add_argument(
        "--right-file-join-columns",
        dest="right_join_columns",
        help="Right file join columns.",
        nargs="+",
    )
    ...
    parser.add_argument(
        "--right-join",
        dest="right_join",
        help="Perform a right outer join (default=%(default)s).",
        type=optional_bool,
        nargs="?",
        const=True,
        default=False,
    ...
    KgtkReaderOptions.add_arguments(
        parser, mode_options=True, who=KgtkJoiner.LEFT, expert=True
    )
    KgtkReaderOptions.add_arguments(
        parser, mode_options=True, who=KgtkJoiner.RIGHT, expert=True
    )
    ...
    left_reader_options: KgtkReaderOptions = KgtkReaderOptions.from_args(
        args, who=KgtkJoiner.LEFT
    )
    right_reader_options: KgtkReaderOptions = KgtkReaderOptions.from_args(
        args, who=KgtkJoiner.RIGHT
    )
    ...
    ej: KgtkJoiner = KgtkJoiner(
        left_file_path=args.left_file_path,
