...
class KgtkReaderMode(Enum):
    ...
    AUTO = (
        3  # Automatically decide whether to enforce edge or node file required columns
    )
...
class KgtkReaderOptions:
    ...
    mode: KgtkReaderMode = attr.ib(
        validator=attr.validators.instance_of(KgtkReaderMode),
        default=KgtkReaderMode.AUTO,
    )

    # The column separator is normally tab.
    column_separator: str = attr.ib(
        validator=attr.validators.instance_of(str), default=KgtkFormat.COLUMN_SEPARATOR
    )

    # supply a missing header record or override an existing header record.
    force_column_names: typing.Optional[typing.List[str]] = attr.ib(
    ...
    skip_first_record: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )

    # How do we handle errors?
    error_limit: int = attr.ib(
        validator=attr.validators.instance_of(int), default=ERROR_LIMIT_DEFAULT
    )  # >0 ==> limit error reports

    # Top-level validation controls:
    repair_and_validate_lines: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    repair_and_validate_values: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )

    # Ignore empty lines, comments, and all whitespace lines, etc.?
    empty_line_action: ValidationAction = attr.ib(
        validator=attr.validators.instance_of(ValidationAction),
        default=ValidationAction.EXCLUDE,
    )
    comment_line_action: ValidationAction = attr.ib(
        validator=attr.validators.instance_of(ValidationAction),
        default=ValidationAction.EXCLUDE,
    )
    whitespace_line_action: ValidationAction = attr.ib(
        validator=attr.validators.instance_of(ValidationAction),
        default=ValidationAction.EXCLUDE,
    )

    # Ignore records with empty values in certain fields:
    blank_required_field_line_action: ValidationAction = attr.ib(
        validator=attr.validators.instance_of(ValidationAction),
        default=ValidationAction.EXCLUDE,
    )

    # Ignore records with too many or too few fields?
    short_line_action: ValidationAction = attr.ib(
        validator=attr.validators.instance_of(ValidationAction),
        default=ValidationAction.COMPLAIN,
    )
    long_line_action: ValidationAction = attr.ib(
        validator=attr.validators.instance_of(ValidationAction),
        default=ValidationAction.COMPLAIN,
    )

    # How should header errors be processed?
    header_error_action: ValidationAction = attr.ib(
        validator=attr.validators.instance_of(ValidationAction),
        default=ValidationAction.EXIT,
    )
    unsafe_column_name_action: ValidationAction = attr.ib(
        validator=attr.validators.instance_of(ValidationAction),
        default=ValidationAction.REPORT,
    )

    # Validate data cell values?
    invalid_value_action: ValidationAction = attr.ib(
        validator=attr.validators.instance_of(ValidationAction),
        default=ValidationAction.COMPLAIN,
    )

    # Repair records with too many or too few fields?
    fill_short_lines: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    truncate_long_lines: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )

    # Other implementation options?
    compression_type: typing.Optional[str] = attr.ib(
        validator=attr.validators.optional(attr.validators.instance_of(str)),
        default=None,
    )  # TODO: use an Enum
    gzip_in_parallel: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    gzip_queue_size: int = attr.ib(
        validator=attr.validators.instance_of(int), default=GZIP_QUEUE_SIZE_DEFAULT
    )
    ...
    def add_arguments(
        ...
        fgroup: _ArgumentGroup = parser.add_argument_group(
            h(prefix3 + "File options"),
            h("Options affecting " + prefix4 + "processing"),
        )
        fgroup.add_argument(
            prefix1 + "column-separator",
            dest=prefix2 + "column_separator",
            help=h(
                prefix3 + "Column separator (default=<TAB>)."
            ),  # TODO: provide the default with escapes, e.g. \t
            type=str,
            **d(default=KgtkFormat.COLUMN_SEPARATOR)
        )

        # TODO: use an Enum or add choices.
        fgroup.add_argument(
            prefix1 + "compression-type",
        ...
        fgroup.add_argument(
            ...
            help=h(
                prefix3
                + "The maximum number of errors to report before failing (default=%(default)s)"
            ),
            type=int,
            **d(default=cls.ERROR_LIMIT_DEFAULT)
        )

        fgroup.add_argument(
            prefix1 + "gzip-in-parallel",
        ...
        if mode_options:
            fgroup.add_argument(
                ...
                **d(KgtkReaderMode.AUTO)
            )

        hgroup: _ArgumentGroup = parser.add_argument_group(
            h(prefix3 + "Header parsing"),
        ...
        hgroup.add_argument(
            prefix1 + "header-error-action",
            dest=prefix2 + "header_error_action",
            help=h(
                prefix3
                + "The action to take when a header error is detected.  Only ERROR or EXIT are supported (default=%(default)s)."
            ),
            type=ValidationAction,
            action=EnumNameAction,
            **d(default=ValidationAction.EXIT)
        )

        hgroup.add_argument(
            prefix1 + "skip-first-record",
            dest=prefix2 + "skip_first_record",
            help=h(
                prefix3
                + "Skip the first record when forcing column names (default=%(default)s)."
            ),
            type=optional_bool,
            nargs="?",
            const=True,
        ...
        hgroup.add_argument(
            prefix1 + "unsafe-column-name-action",
            dest=prefix2 + "unsafe_column_name_action",
            help=h(
                prefix3
                + "The action to take when a column name is unsafe (default=%(default)s)."
            ),
            type=ValidationAction,
            action=EnumNameAction,
            **d(default=ValidationAction.REPORT)
        )

        lgroup: _ArgumentGroup = parser.add_argument_group(
            h(prefix3 + "Line parsing"),
            h("Options affecting " + prefix4 + "data line parsing"),
        )

        lgroup.add_argument(
            prefix1 + "repair-and-validate-lines",
            dest=prefix2 + "repair_and_validate_lines",
            help=h(prefix3 + "Repair and validate lines (default=%(default)s)."),
            type=optional_bool,
            nargs="?",
            const=True,
            **d(default=validate_by_default)
        )

        lgroup.add_argument(
            prefix1 + "repair-and-validate-values",
        ...
        lgroup.add_argument(
            prefix1 + "blank-required-field-line-action",
            dest=prefix2 + "blank_required_field_line_action",
            help=h(
                prefix3
                + "The action to take when a line with a blank node1, node2, or id field (per mode) is detected (default=%(default)s)."
            ),
            type=ValidationAction,
            action=EnumNameAction,
            **d(default=ValidationAction.EXCLUDE)
        )

        lgroup.add_argument(
            prefix1 + "comment-line-action",
            dest=prefix2 + "comment_line_action",
            help=h(
                prefix3
                + "The action to take when a comment line is detected (default=%(default)s)."
            ),
            type=ValidationAction,
            action=EnumNameAction,
            **d(default=ValidationAction.EXCLUDE)
        ...
        lgroup.add_argument(
            prefix1 + "empty-line-action",
            dest=prefix2 + "empty_line_action",
            help=h(
                prefix3
                + "The action to take when an empty line is detected (default=%(default)s)."
            ),
            type=ValidationAction,
            action=EnumNameAction,
            **d(default=ValidationAction.EXCLUDE)
        ...
        lgroup.add_argument(
            prefix1 + "fill-short-lines",
            dest=prefix2 + "fill_short_lines",
            help=h(
                prefix3
                + "Fill missing trailing columns in short lines with empty values (default=%(default)s)."
            ),
            type=optional_bool,
            nargs="?",
            const=True,
        ...
        lgroup.add_argument(
            prefix1 + "invalid-value-action",
            dest=prefix2 + "invalid_value_action",
            help=h(
                prefix3
                + "The action to take when a data cell value is invalid (default=%(default)s)."
            ),
            type=ValidationAction,
            action=EnumNameAction,
            **d(default=ValidationAction.COMPLAIN)
        )

        lgroup.add_argument(
            prefix1 + "long-line-action",
            dest=prefix2 + "long_line_action",
            help=h(
                prefix3
                + "The action to take when a long line is detected (default=%(default)s)."
            ),
            type=ValidationAction,
            action=EnumNameAction,
            **d(default=ValidationAction.COMPLAIN)
        ...
        lgroup.add_argument(
            prefix1 + "short-line-action",
            dest=prefix2 + "short_line_action",
            help=h(
                prefix3
                + "The action to take when a short line is detected (default=%(default)s)."
            ),
            type=ValidationAction,
            action=EnumNameAction,
            **d(default=ValidationAction.COMPLAIN)
        ...
        lgroup.add_argument(
            prefix1 + "truncate-long-lines",
            dest=prefix2 + "truncate_long_lines",
            help=h(
                prefix3
                + "Remove excess trailing columns in long lines (default=%(default)s)."
            ),
            type=optional_bool,
            nargs="?",
            const=True,
        ...
        lgroup.add_argument(
            prefix1 + "whitespace-line-action",
            dest=prefix2 + "whitespace_line_action",
            help=h(
                prefix3
                + "The action to take when a whitespace line is detected (default=%(default)s)."
            ),
            type=ValidationAction,
            action=EnumNameAction,
            **d(default=ValidationAction.EXCLUDE)
    ...
    def from_dict(
        ...
        return cls(
            blank_required_field_line_action=lookup(
                "blank_required_field_line_action", ValidationAction.EXCLUDE
            ),
            ...
            gzip_queue_size=lookup(
                "gzip_queue_size", KgtkReaderOptions.GZIP_QUEUE_SIZE_DEFAULT
            ),
            header_error_action=lookup("header_error_action", ValidationAction.EXCLUDE),
            invalid_value_action=lookup(
                "invalid_value_action", ValidationAction.REPORT
            ),
            ...
            unsafe_column_name_action=lookup(
                "unsafe_column_name_action", ValidationAction.REPORT
            ),
            whitespace_line_action=lookup(
                "whitespace_line_action", ValidationAction.EXCLUDE
            ),
        )
    ...
    def show(self, who: str = "", out: typing.TextIO = sys.stderr):
        ...
        if self.force_column_names is not None:
            print(
                "%sforce_column_names=%s" % (prefix, " ".join(self.force_column_names)),
                file=out,
            )
        print(
            "%sskip_first_record=%s" % (prefix, str(self.skip_first_record)), file=out
        )
        print("%serror-limit=%s" % (prefix, str(self.error_limit)), file=out)
        print(
            "%srepair-and-validate-lines=%s"
            % (prefix, str(self.repair_and_validate_lines)),
            file=out,
        )
        print(
            "%srepair-and-validate-values=%s"
            % (prefix, str(self.repair_and_validate_values)),
            file=out,
        )
        print(
            "%sempty-line-action=%s" % (prefix, self.empty_line_action.name), file=out
        )
        print(
            "%scomment-line-action=%s" % (prefix, self.comment_line_action.name),
            file=out,
        )
        print(
            "%swhitespace-line-action=%s" % (prefix, self.whitespace_line_action.name),
            file=out,
        )
        print(
            "%sblank-required-field-line-action=%s"
            % (prefix, self.blank_required_field_line_action.name),
            file=out,
        )
        print(
            "%sshort-line-action=%s" % (prefix, self.short_line_action.name), file=out
        )
        print("%slong-line-action=%s" % (prefix, self.long_line_action.name), file=out)
        print(
            "%sheader-error-action=%s" % (prefix, self.header_error_action.name),
            file=out,
        )
        print(
            "%sunsafe-column-name-action=%s"
            % (prefix, self.unsafe_column_name_action.name),
            file=out,
        )
        print(
            "%sinvalid-value-action=%s" % (prefix, self.invalid_value_action.name),
            file=out,
        )
        print("%sfill-short-lines=%s" % (prefix, str(self.fill_short_lines)), file=out)
        print(
            "%struncate-long-lines=%s" % (prefix, str(self.truncate_long_lines)),
            file=out,
        )
        if self.compression_type is not None:
            print(
                "%scompression-type=%s" % (prefix, str(self.compression_type)), file=out
            )
...
class KgtkReader(KgtkBase, ClosableIter[typing.List[str]]):
    file_path: typing.Optional[Path] = attr.ib(
    ...
    node1_column_idx: int = attr.ib(
        validator=attr.validators.instance_of(int), default=-1
    )  # edge file
    label_column_idx: int = attr.ib(
        validator=attr.validators.instance_of(int), default=-1
    )  # edge file
    node2_column_idx: int = attr.ib(
        validator=attr.validators.instance_of(int), default=-1
    )  # edge file
    id_column_idx: int = attr.ib(
        validator=attr.validators.instance_of(int), default=-1
    )  # node file

    data_lines_read: int = attr.ib(
        validator=attr.validators.instance_of(int), default=0
    )
    data_lines_passed: int = attr.ib(
        validator=attr.validators.instance_of(int), default=0
    )
    data_lines_ignored: int = attr.ib(
        validator=attr.validators.instance_of(int), default=0
    )
    data_errors_reported: int = attr.ib(
        validator=attr.validators.instance_of(int), default=0
    )

    # Is this an edge file or a node file?
    is_edge_file: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    is_node_file: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    ...
    def open(
        ...
        source: ClosableIter[str] = cls._openfile(
            file_path, options=options, error_file=error_file, verbose=verbose
        )
        ...
        (header, column_names) = cls._build_column_names(
            source, options, error_file=error_file, verbose=verbose
        )
        ...
        column_name_map: typing.Mapping[str, int] = cls.build_column_name_map(
            column_names,
            header_line=header,
            who=who,
        ...
        if mode is KgtkReaderMode.AUTO:
            ...
            node1_idx: int = cls.get_column_idx(
                cls.NODE1_COLUMN_NAMES,
                column_name_map,
                header_line=header,
                who=who,
                error_action=options.header_error_action,
            ...
            if node1_idx >= 0:
                ...
                if verbose:
                    print(
                        "%s column found, this is a KGTK edge file"
                        % column_names[node1_idx],
                        file=error_file,
                        flush=True,
                    )
            else:
        ...
        if verbose:
            print(
                "KgtkReader: Special columns: node1=%d label=%d node2=%d id=%d"
    ...
    def _open_compressed_file(
        ...
        if compression_type in [".gz", "gz"]:
            if verbose:
                print(
                    "%s: reading gzip %s" % (who, file_name),
                    file=error_file,
                    flush=True,
                )
            ...
            if verbose:
                print(
                    "%s: reading bz2 %s" % (who, file_name), file=error_file, flush=True
                )
            ...
            if verbose:
                print(
                    "%s: reading lzma %s" % (who, file_name),
                    file=error_file,
                    flush=True,
                )
            ...
            if verbose:
                print(
                    "%s: reading lz4 %s" % (who, file_name), file=error_file, flush=True
                )
            return lz4.frame.open(file_or_path, mode="rt")  # type: ignore
        else:
            # TODO: throw a better exception.
            raise ValueError(
                "%s: Unexpected compression_type '%s'" % (who, compression_type)
            )
    ...
    def _openfile(
        ...
        if file_path is None or str(file_path) == "-":
            if (
                options.compression_type is not None
                and len(options.compression_type) > 0
            ):
                return ClosableIterTextIOWrapper(
        ...
        if verbose:
            print(
                "%s: File_path.suffix: %s" % (who, file_path.suffix),
                file=error_file,
                flush=True,
            )
        ...
        if options.compression_type is not None and len(options.compression_type) > 0:
            gzip_file = cls._open_compressed_file(
                options.compression_type,
                str(file_path),
                file_path,
                who,
            ...
            gzip_file = cls._open_compressed_file(
                file_path.suffix, str(file_path), file_path, who, error_file, verbose
            )
        else:
            if verbose:
        ...
        if options.gzip_in_parallel:
            gzip_thread: GunzipProcess = GunzipProcess(
                gzip_file, Queue(options.gzip_queue_size)
            )
            gzip_thread.start()
            return gzip_thread
        else:
    ...
    def _build_column_names(
        ...
        if options.force_column_names is None:
            ...
            return (
                options.column_separator.join(options.force_column_names),
                options.force_column_names,
            )
    ...
    def exclude_line(self, action: ValidationAction, msg: str, line: str) -> bool:
        ...
        if action == ValidationAction.PASS:
            ...
            raise ValueError(
                "In input data line %d, %s: %s" % (self.data_lines_read, msg, line)
            )
        elif action == ValidationAction.EXIT:
            print(
                "Data line %d:\n%s\n%s" % (self.data_lines_read, line, msg),
        ...
        if (
            self.options.error_limit > 0
            and self.data_errors_reported >= self.options.error_limit
        ):
    ...
    def nextrow(self) -> typing.List[str]:
        ...
        while True:
            ...
            if repair_and_validate_lines:
                ...
                # Ignore empty lines.
                if (
                    self.options.empty_line_action != ValidationAction.PASS
                    and len(line) == 0
                ):
                    if self.exclude_line(
                        self.options.empty_line_action, "saw an empty line", line
                    ):
                        continue

                # Ignore comment lines:
                if (
                    self.options.comment_line_action != ValidationAction.PASS
                    and line[0] == self.COMMENT_INDICATOR
                ):
                    if self.exclude_line(
                        self.options.comment_line_action, "saw a comment line", line
                    ):
                        continue

                # Ignore whitespace lines
                if (
                    self.options.whitespace_line_action != ValidationAction.PASS
                    and line.isspace()
                ):
                    if self.exclude_line(
                        self.options.whitespace_line_action,
                        "saw a whitespace line",
                        line,
                    ):
                        continue
            ...
            if repair_and_validate_lines:
                ...
                if (
                    self.options.short_line_action != ValidationAction.PASS
                    and len(row) < self.column_count
                ):
                    if self.exclude_line(
                        self.options.short_line_action,
                        "Required %d columns, saw %d: '%s'"
                        % (self.column_count, len(row), line),
                        line,
                    ):
                        continue

                if (
                    self.options.long_line_action != ValidationAction.PASS
                    and len(row) > self.column_count
                ):
                    if self.exclude_line(
                        self.options.long_line_action,
                        "Required %d columns, saw %d (%d extra): '%s'"
                        % (
                            self.column_count,
                            len(row),
            ...
            if (
                repair_and_validate_values
                and self.options.invalid_value_action != ValidationAction.PASS
            ):
    ...
    def to_kgtk_values(
        self, row: typing.List[str], validate: bool = False, parse_fields: bool = False
    ) -> typing.List[KgtkValue]:
        """
    ...
    def kgtk_values(
        self, validate: bool = False, parse_fields: bool = False
        ...
        while True:
            try:
                yield self.to_kgtk_values(
                    self.nextrow(), validate=validate, parse_fields=parse_fields
                )
            except StopIteration:
                return

    def to_concise_kgtk_values(
        self, row: typing.List[str], validate: bool = False, parse_fields: bool = False
        ...
        for field in row:
            if len(field) == 0:
                ...
                kv = KgtkValue(
                    field, options=self.value_options, parse_fields=parse_fields
                )
                if validate:
                    kv.validate()
                results.append(kv)
    ...
    def to_dict(
        self, row: typing.List[str], concise: bool = False
    ...
    def kgtk_value_dicts(
        self, validate: bool = False, parse_fields: bool = False, concise: bool = False
    ...
    def _ignore_invalid_values(self, row: typing.List[str], line: str) -> bool:
        ...
        for idx, item in enumerate(row):
            if len(item) > 0:  # Optimize the common case of empty columns.
                ...
                if not kv.is_valid():
                    problems.append(
                        "col %d (%s) value '%s'is an %s"
                        % (idx, self.column_names[idx], item, kv.describe())
                    )
                if kv.repaired:
        ...
        return self.exclude_line(
            self.options.invalid_value_action, "\n".join(problems), line
        )
    ...
    def add_debug_arguments(cls, parser: ArgumentParser, expert: bool = False):
        ...
        egroup: _ArgumentGroup = parser.add_argument_group(
            h("Error and feedback messages"),
            h(
                "Send error messages and feedback to stderr or stdout, "
...
def main():
    ...
    parser = ArgumentParser()
    parser.add_argument(
        dest="kgtk_file", help="The KGTK file to read", type=Path, nargs="?"
    )
    KgtkReader.add_debug_arguments(parser, expert=True)
    parser.add_argument(
        "--test",
        dest="test_method",
        help="The test to perform (default=%(default)s).",
        choices=[
            "rows",
            "concise-rows",
            "kgtk-values",
    ...
    parser.add_argument(
        "--test-validate",
        dest="test_validate",
        help="Validate KgtkValue objects in test (default=%(default)s).",
        type=optional_bool,
        nargs="?",
        const=True,
        default=False,
    ...
    KgtkReaderOptions.add_arguments(
        parser, mode_options=True, validate_by_default=True, expert=True
    )
    ...
    if args.test_method == "rows":
        ...
        if args.verbose:
            print(
                "Testing iterating over concise KgtkValue rows.",
                file=error_file,
                flush=True,
            )
        ...
        if args.verbose:
            print(
                "Testing iterating over KgtkValue dicts.", file=error_file, flush=True
            )
        ...
        if args.verbose:
            print(
                "Testing iterating over concise KgtkValue dicts.",
                file=error_file,
                flush=True,
            )
        for kgtk_value_dict in kr.kgtk_value_dicts(
            concise=True, validate=args.test_validate
        ):
            line_count += 1
