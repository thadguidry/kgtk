...
class KgtkValueOptions:
    ...
    allow_month_or_day_zero: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    repair_month_or_day_zero: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    ...
    allow_lax_strings: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    ...
    allow_lax_lq_strings: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )

    allow_language_suffixes: bool = attr.ib(
    ...
    minimum_valid_year: int = attr.ib(
        validator=attr.validators.instance_of(int), default=MINIMUM_VALID_YEAR
    )
    clamp_minimum_year: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )

    MAXIMUM_VALID_YEAR: int = 2100  # Arbitrarily chosen.
    maximum_valid_year: int = attr.ib(
        validator=attr.validators.instance_of(int), default=MAXIMUM_VALID_YEAR
    )
    clamp_maximum_year: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )

    MINIMUM_VALID_LAT: float = -90.0
    minimum_valid_lat: float = attr.ib(
        validator=attr.validators.instance_of(float), default=MINIMUM_VALID_LAT
    )
    clamp_minimum_lat: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )

    MAXIMUM_VALID_LAT: float = 90.0
    maximum_valid_lat: float = attr.ib(
        validator=attr.validators.instance_of(float), default=MAXIMUM_VALID_LAT
    )
    clamp_maximum_lat: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )

    MINIMUM_VALID_LON: float = -180.0
    minimum_valid_lon: float = attr.ib(
        validator=attr.validators.instance_of(float), default=MINIMUM_VALID_LON
    )
    clamp_minimum_lon: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )

    MAXIMUM_VALID_LON: float = 180.0
    maximum_valid_lon: float = attr.ib(
        validator=attr.validators.instance_of(float), default=MAXIMUM_VALID_LON
    )
    clamp_maximum_lon: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )

    modulo_repair_lon: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    ...
    def add_arguments(
        ...
        vgroup.add_argument(
            ...
            help=h(
                prefix3 + "Allow month or day zero in dates. (default=%(default)s)."
            ),
            type=optional_bool,
            nargs="?",
            const=True,
        ...
        vgroup.add_argument(
            ...
            help=h(
                prefix3 + "Repair month or day zero in dates. (default=%(default)s)."
            ),
            type=optional_bool,
            nargs="?",
            const=True,
        ...
        vgroup.add_argument(
            ...
            help=h(
                prefix3 + "Clamp years at the minimum value. (default=%(default)s)."
            ),
            type=optional_bool,
            nargs="?",
            const=True,
        ...
        vgroup.add_argument(
            ...
            help=h(
                prefix3 + "Clamp years at the maximum value. (default=%(default)s)."
            ),
            type=optional_bool,
            nargs="?",
            const=True,
        ...
        vgroup.add_argument(
            ...
            help=h(
                prefix3 + "Clamp latitudes at the minimum value. (default=%(default)s)."
            ),
            type=optional_bool,
            nargs="?",
            const=True,
        ...
        vgroup.add_argument(
            ...
            help=h(
                prefix3 + "Clamp latitudes at the maximum value. (default=%(default)s)."
            ),
            type=optional_bool,
            nargs="?",
            const=True,
        ...
        vgroup.add_argument(
            ...
            help=h(
                prefix3 + "Wrap longitude to (-180.0,180.0]. (default=%(default)s)."
            ),
            type=optional_bool,
            nargs="?",
            const=True,
    ...
    def from_dict(cls, d: dict, who: str = "") -> "KgtkValueOptions":
        ...
        return cls(
            ...
            minimum_valid_year=d.get(
                prefix + "minimum_valid_year", cls.MINIMUM_VALID_YEAR
            ),
            clamp_minimum_year=d.get(prefix + "clamp_minimum_year", False),
            maximum_valid_year=d.get(
                prefix + "maximum_valid_year", cls.MAXIMUM_VALID_YEAR
            ),
            ...
            minimum_valid_lat=d.get(
                prefix + "minimum_valid_lat", cls.MINIMUM_VALID_LAT
            ),
            clamp_minimum_lat=d.get(prefix + "clamp_minimum_lat", False),
            maximum_valid_lat=d.get(
                prefix + "maximum_valid_lat", cls.MAXIMUM_VALID_LAT
            ),
            clamp_maximum_lat=d.get(prefix + "clamp_maximum_lat", False),
            minimum_valid_lon=d.get(
                prefix + "minimum_valid_lon", cls.MINIMUM_VALID_LON
            ),
            clamp_minimum_lon=d.get(prefix + "clamp_minimum_lon", False),
            maximum_valid_lon=d.get(
                prefix + "maximum_valid_lon", cls.MAXIMUM_VALID_LON
            ),
    ...
    def show(self, who: str = "", out: typing.TextIO = sys.stderr):
        ...
        print(
            "%sallow-month-or-day-zero=%s"
            % (prefix, str(self.allow_month_or_day_zero)),
            file=out,
        )
        print(
            "%srepair-month-or-day-zero=%s"
            % (prefix, str(self.repair_month_or_day_zero)),
            file=out,
        )
        print(
            "%sallow-language-suffixes=%s"
            % (prefix, str(self.allow_language_suffixes)),
            file=out,
        )
        print(
            "%sallow-lax-strings=%s" % (prefix, str(self.allow_lax_strings)), file=out
        )
        print(
            "%sallow-lax-lq-strings=%s" % (prefix, str(self.allow_lax_lq_strings)),
            file=out,
        )
        if self.additional_language_codes is not None:
            print(
                "%sadditional-language-codes=%s"
                % (prefix, " ".join(self.additional_language_codes)),
                file=out,
            )

        print("%sminimum-valid-year=%d" % (prefix, self.minimum_valid_year), file=out)
        print(
            "%sclamp-minimum-year=%s" % (prefix, str(self.clamp_minimum_year)), file=out
        )
        print("%smaximum-valid-year=%d" % (prefix, self.maximum_valid_year), file=out)
        print(
            "%sclamp-maximum-year=%s" % (prefix, str(self.clamp_maximum_year)), file=out
        )

        print(
            "%sallow-lax-coordinates=%s" % (prefix, str(self.allow_lax_coordinates)),
            file=out,
        )
        print(
            "%srepair-lax-coordinates=%s" % (prefix, str(self.repair_lax_coordinates)),
            file=out,
        )

        print("%sminimum-valid-lat=%f" % (prefix, self.minimum_valid_lat), file=out)
        print(
            "%sclamp-minimum-lat=%s" % (prefix, str(self.clamp_minimum_lat)), file=out
        )
        print("%smaximum-valid-lat=%f" % (prefix, self.maximum_valid_lat), file=out)
        print(
            "%sclamp-maximum-lat=%s" % (prefix, str(self.clamp_maximum_lat)), file=out
        )

        print("%sminimum-valid-lon=%f" % (prefix, self.minimum_valid_lon), file=out)
        print(
            "%sclamp-minimum-lon=%s" % (prefix, str(self.clamp_minimum_lon)), file=out
        )
        print("%smaximum-valid-lon=%f" % (prefix, self.maximum_valid_lon), file=out)
        print(
            "%sclamp-maximum-lon=%s" % (prefix, str(self.clamp_maximum_lon)), file=out
        )
        print(
            "%smodulo-repair-lon=%s" % (prefix, str(self.modulo_repair_lon)), file=out
        )

        print(
            "%sescape-list-separators=%s" % (prefix, str(self.escape_list_separators)),
            file=out,
        )
...
def main():
    ...
    KgtkValueOptions.add_arguments(
        parser, who="left", desc=" for the left file.", expert=True
    )
    KgtkValueOptions.add_arguments(
        parser, who="right", desc=" for the right file.", expert=True
    )
    ...
    print(
        "left_allow_month_or_day_zero: %s"
        % str(left_value_options.allow_month_or_day_zero)
    )

    right_value_options: KgtkValueOptions = KgtkValueOptions.from_args(
        args, who="right"
    )
    print(
        "right_allow_month_or_day_zero: %s"
        % str(right_value_options.allow_month_or_day_zero)
    )
