...
def parser():
    return {"help": "Import ConceptNet into KGTK."}
...
def add_arguments(parser):
    ...
    parser.add_argument(
        "--english_only", action="store_true", help="Only english conceptnet?"
    )
    parser.add_argument(
        "--sort", action="store_true", help="Should we sort the file on s-p-o?"
    )
    ...
    try:
        ...
        with open(filename, "r") as f:
            ...
            for row in reader:
                ...
                if not english_only or (
                    new_row[0].startswith("/c/en/") and new_row[2].startswith("/c/en/")
                ):
