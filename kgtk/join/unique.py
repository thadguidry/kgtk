...
class Unique(KgtkFormat):
    ...
    label_value: str = attr.ib(
        validator=attr.validators.instance_of(str), default="count"
    )

    # TODO: make this an enum
    output_format: str = attr.ib(
        validator=attr.validators.instance_of(str), default="edge"
    )
    ...
    def process(self):
        ...
        if self.verbose:
            print(
                "Counting unique values from the %s column in %s"
        ...
        if self.verbose:
            print(
                "Read %d records, found %d unique non-empty values, %d empty values."
                % (
                    input_line_count,
                    len(value_counts),
