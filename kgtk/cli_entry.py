...
handlers = [
    x.name for x in pkgutil.iter_modules(cli.__path__) if not x.name.startswith("__")
]
    ...
    if len(pipe) == 0:
        ...
        for idx, cmd_args in enumerate(pipe):
            ...
            if idx == 0:  # first command
                concat_cmd_str = "sh.kgtk({}, _in=sys.stdin, _piped=True)".format(
                    cmd_str
                )
            elif idx + 1 == len(pipe):  # last command
                concat_cmd_str = "sh.kgtk({}, {}, _out=sys.stdout)".format(
                    concat_cmd_str, cmd_str
                )
            else:
                concat_cmd_str = "sh.kgtk({}, {}, _piped=True)".format(
                    concat_cmd_str, cmd_str
                )
        try:
            ...
            # mimic parser exit
            parser.exit(
                KGTKArgumentParseException.return_code, e.stderr.decode("utf-8")
            )
