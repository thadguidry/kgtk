...
class KgtkExplode(KgtkFormat):
    ...
    column_name: str = attr.ib(
        validator=attr.validators.instance_of(str), default="node2"
    )
    ...
    type_names: typing.Optional[typing.List[str]] = attr.ib(
        validator=attr.validators.optional(
            attr.validators.deep_iterable(
                member_validator=attr.validators.instance_of(str),
                iterable_validator=attr.validators.instance_of(list),
    ...
    overwrite_columns: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    expand_list: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    ...
    def process(self):
        ...
        if self.type_names is not None:
            if self.verbose:
                print(
                    "Validate the names of the data types to extract.",
        ...
        if self.field_names is not None:
            ...
            if self.verbose:
                print(
                    "Validate the names of the fields to extract.",
                    file=self.error_file,
                    flush=True,
                )
        ...
        if self.verbose:
            print(
                "Check that the source column '%s' is present." % self.column_name,
                file=self.error_file,
                flush=True,
            )
        if self.column_name not in kr.column_name_map:
            raise ValueError(
                "Column name '%s' not found in the input file." % self.column_name
            )
        ...
        for field_name in selected_field_names:
            ...
            if self.verbose:
                print(
                    "Field '%s' becomes '%s'" % (field_name, exploded_name),
                    file=self.error_file,
                    flush=True,
                )
            ...
            if exploded_name in kr.column_names:
                ...
                if self.verbose:
                    print(
                        "Field '%s' becomes new column '%s' (idx=%d)"
        ...
        for row in kr:
            ...
            if not value.is_valid():
                if self.verbose:
                    print(
                        "Not exploding invalid item '%s' in input line %d"
            ...
            if self.expand_list and value.is_list():
                if self.verbose:
                    print(
                        "Expanding a list: '%s'" % item_to_explode,
                        file=self.error_file,
                        flush=True,
                    )
                subvalue: KgtkValue
    ...
    def explode(
        ...
        if new_column_count > 0:
            ...
            for i in range(new_column_count):
                newrow.append("")
        field_map: typing.Mapping[
            str, typing.Union[str, int, float, bool]
        ] = value.get_field_map()
        field_name: str
        idx: int
...
def main():
    ...
    if args.show_options:
        ...
        print(
            "--overwrite %s" % str(args.overwrite_columns), file=error_file, flush=True
        )
        print("--expand %s" % str(args.expand_list), file=error_file, flush=True)
        if args.field_names is not None:
            print(
                "--fields %s" % " ".join(args.field_names), file=error_file, flush=True
            )
