...
class KgtkIfExists(KgtkFormat):
    ...
    input_keys: typing.Optional[typing.List[str]] = attr.ib(
    ...
    filter_keys: typing.Optional[typing.List[str]] = attr.ib(
        validator=attr.validators.optional(
            attr.validators.deep_iterable(
                member_validator=attr.validators.instance_of(str),
                iterable_validator=attr.validators.instance_of(list),
    ...
    cache_input: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    preserve_order: bool = attr.ib(
        validator=attr.validators.instance_of(bool), default=False
    )
    ...
    def get_primary_key_column(self, kr: KgtkReader, who: str) -> typing.List[int]:
        if kr.is_node_file:
            if kr.id_column_idx < 0:
                raise ValueError(
                    "The id column is missing from the %s node file." % who
                )
            return [kr.id_column_idx]
        elif kr.is_edge_file:
            if kr.node1_column_idx < 0:
                raise ValueError(
                    "The node1 column is missing from the %s node file." % who
                )
    ...
    def get_supplied_key_columns(
        self, supplied_keys: typing.List[str], kr: KgtkReader, who: str
    ) -> typing.List[int]:
    ...
    def get_key_columns(
        self,
        supplied_keys: typing.Optional[typing.List[str]],
        kr: KgtkReader,
        other_kr: KgtkReader,
        who: str,
    ) -> typing.List[int]:
        ...
        if not (kr.is_node_file or kr.is_edge_file):
            raise ValueError(
                "The %s file is a quasi-KGTK file.  Please supply its keys." % who
            )
    ...
    def extract_key_set(
        self, kr: KgtkReader, who: str, key_columns: typing.List[int]
    ) -> typing.Set[str]:
    ...
    def extract_key_set_and_cache(
        self, kr: KgtkReader, who: str, key_columns: typing.List[int]
    ) -> typing.Tuple[typing.Set[str], typing.List[typing.List[str]]]:
        key_set: typing.Set[str] = set()
    ...
    def process_cacheing_filter(
        ...
        if self.verbose:
            print(
                "Building the filter key set from %s" % self.filter_file_path,
        ...
        if self.verbose or self.very_verbose:
            print(
                "There are %d entries in the filter key set." % len(key_set),
                file=self.error_file,
                flush=True,
            )
        ...
        if self.verbose:
            print(
                "Filtering records from %s" % self.input_file_path,
                file=self.error_file,
                flush=True,
            )
    ...
    def process_cacheing_input(
        ...
        if self.verbose:
            print(
                "Reading the input data from %s" % self.input_file_path,
                file=self.error_file,
                flush=True,
            )
        row: typing.List[str]
        ...
        if self.verbose:
            print(
                "Applying the filter from %s" % self.filter_file_path,
                file=self.error_file,
                flush=True,
            )
        filter_key: str
        ...
        if self.verbose:
            print(
                "Writing the output data to %s" % self.output_file_path,
                file=self.error_file,
                flush=True,
            )
    ...
    def process_cacheing_input_preserving_order(
        ...
        if self.verbose:
            print(
                "Building the input key set from %s" % self.input_file_path,
                file=self.error_file,
                flush=True,
            )
        ...
        if self.verbose or self.very_verbose:
            print(
                "There are %d rows in the input cache." % input_line_count,
                file=self.error_file,
                flush=True,
            )
            print(
                "There are %d entries in the input key set." % len(input_key_set),
                file=self.error_file,
                flush=True,
            )
            if self.very_verbose:
                print(
                    "Keys: %s" % " ".join(input_key_set),
                    file=self.error_file,
                    flush=True,
                )
        ...
        if self.verbose:
            print(
                "Read %d rows from the filter file." % filter_line_count,
                file=self.error_file,
                flush=True,
            )
            print(
                "There are %d entries in the output key set." % len(output_key_set),
                file=self.error_file,
                flush=True,
            )
        ...
        if self.verbose:
            print(
                "Wrote %d rows to the output file." % output_line_count,
                file=self.error_file,
                flush=True,
            )
    ...
    def process(self):
        ...
        if self.verbose:
            print(
                "Opening the filter input file: %s" % self.filter_file_path, flush=True
            )
        filter_kr: KgtkReader = KgtkReader.open(
        ...
        input_key_columns: typing.List[int] = self.get_key_columns(
            self.input_keys, input_kr, filter_kr, "input"
        )
        filter_key_columns: typing.List[int] = self.get_key_columns(
            self.filter_keys, filter_kr, input_kr, "filter"
        )

        if len(input_key_columns) != len(filter_key_columns):
            print(
                "There are %d input key columns but %d filter key columns.  Exiting."
                % (len(input_key_columns), len(filter_key_columns)),
                file=self.error_file,
                flush=True,
            )
            return
        ...
        if self.verbose:
            print(
                "Opening the output file: %s" % self.output_file_path,
                file=self.error_file,
                flush=True,
            )
        ew: KgtkWriter = KgtkWriter.open(
            input_kr.column_names,
        ...
        if self.cache_input:
            if self.preserve_order:
                pass
                self.process_cacheing_input_preserving_order(
                    input_kr=input_kr,
                    filter_kr=filter_kr,
...
def main():
    ...
    if args.show_options:
        print(
            "input: %s"
            % (str(args.input_file_path) if args.input_file_path is not None else "-"),
            file=error_file,
        )
