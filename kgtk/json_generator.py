...
class JsonGenerator:
    ...
    def update_label_json_dict(self, node1: str, prop: str, node2: str):
        ...
        if node1 not in self.prop_types:
            label_type = "item"
            self.label_json_dict[node1] = {"type": label_type}
        else:
            label_type = "property"
    ...
    def update_info_json_dict(self, node: str, data_type=None):
        ...
        if node.startswith("Q"):
            ...
            raise KGTKException(
                "node {} is neither an entity nor a property.".format(node)
            )

    def update_misc_json_dict(
        self, node1: str, prop: str, node2: str, line_number: int, field: str
    ):
        if node1 not in self.misc_json_dict:
            self.misc_json_dict[node1] = {
                **self.label_json_dict[node1],
                **self.info_json_dict[node1],
            }
        ...
        if prop not in self.prop_types:
            raise KGTKException(
                "property {} at line {} is not defined.".format(prop, line_number)
            )
        ...
        try:
            if self.prop_types[prop] == "wikibase-item":
                ...
                raise KGTKException(
                    "property tyepe {} of property {} at line {} is not defined.".format(
                        self.prop_types[prop], prop, line_number
                    )
                )
        except:
            ...
            "mainsnak": {
                ...
                "datavalue": {
                    "value": {
                        "entity-type": "item",
                        "numeric-id": 0,
                        "id": node2,  # place holder for numeric id
                    },
    ...
    def update_misc_json_dict_monolingualtext(self, node1, prop, node2):
        ...
        temp_mono_dict = {
            "mainsnak": {
                ...
                "hash": "hashplaceholder",
                "datavalue": {
                    "value": {"text": text_string, "language": lang},
    ...
    def set_sets(self, label_set: str, alias_set: str, description_set: str):
        self.label_set, self.alias_set, self.description_set = (
            set(label_set.split(",")),
            set(alias_set.split(",")),
