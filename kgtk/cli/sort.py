...
def parser():
    return {"help": "Sort file based on one or more columns"}
...
def build_command(
    ...
    if isinstance(output, str):
        ...
        output = outfile
    in_bufsize, out_bufsize = zcat.get_buf_sizes(
        output=None, _tty_out=False, _piped=True
    )
    ...
    sort_pipe.append(
        sh.tail.bake(
            "-n", "+2", _piped=True, _in_bufsize=in_bufsize, _out_bufsize=out_bufsize
        )
    )
    sort_pipe.append(
        sh.sh.bake(
            "-c",
            """exec sort -t "%s" %s `cat $KGTK_SORT_KEY_SPEC`""" % (colsep, options),
            _env=sort_env,
            _piped=True,
            _preexec_fn=wait_for_key_spec,
            _in_bufsize=in_bufsize,
            _out_bufsize=out_bufsize,
        )
    )
    in_bufsize, out_bufsize = zcat.get_buf_sizes(output=output, _piped=_piped)
    cleanup = lambda cmd, status, exit_code: [
        sh.rm("-f", sort_env["KGTK_HEADER"]),
        sh.rm("-f", sort_env["KGTK_SORT_KEY_SPEC"]),
    ]
    sort_pipe.append(
        sh.sh.bake(
            "-c",
            """exec cat $KGTK_HEADER -""",
            _env=sort_env,
            _out=output,
            _done=cleanup,
            _piped=_piped,
...
def run(
    ...
    try:
        ...
        pipe = build_command(
            input=input, output=output, columns=columns, colsep=colsep, options=options
        )
        ...
        raise KGTKException(
            "INTERNAL ERROR: " + type(e).__module__ + "." + str(e) + "\n"
        )
