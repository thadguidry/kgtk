...
class NodeReader(KgtkReader):
    ...
    def _ignore_if_blank_required_fields(
        ...
        if (
            ...
            and self.id_column_idx >= 0
            and len(values) > self.id_column_idx
        ):
            id_value: str = values[self.id_column_idx]
            if len(id_value) == 0 or id_value.isspace():
                return self.exclude_line(
                    self.options.blank_required_field_line_action, "id is blank", line
                )
...
def main():
    ...
    nr: NodeReader = NodeReader.open_node_file(
        args.kgtk_file,
        error_file=error_file,
