...
class EdgeReader(KgtkReader):
    ...
    def _ignore_if_blank_required_fields(
        ...
        # to come first.
        if (
            self.options.blank_required_field_line_action != ValidationAction.PASS
            and self.node1_column_idx >= 0
            and len(values) > self.node1_column_idx
        ):
            node1_value: str = values[self.node1_column_idx]
            if len(node1_value) == 0 or node1_value.isspace():
                return self.exclude_line(
                    self.options.blank_required_field_line_action,
                    "node1 is blank",
                    line,
                )
        ...
        if (
            self.options.blank_required_field_line_action != ValidationAction.PASS
            and self.node2_column_idx >= 0
            and len(values) > self.node2_column_idx
        ):
...
def main():
    ...
    parser.add_argument(
        dest="kgtk_file", help="The KGTK edge file to read", type=Path, nargs="?"
    )
    ...
    reader_options: KgtkReaderOptions = KgtkReaderOptions.from_args(
        args, mode=KgtkReaderMode.EDGE
    )
    ...
    # Force the edge mode:
    er: EdgeReader = EdgeReader.open_edge_file(
        args.kgtk_file,
        error_file=error_file,
