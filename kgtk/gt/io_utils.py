...
def transform_to_graphtool_format(
    mowgli_nodes_path: str, mowgli_edges_path: str, graphml_path: str, do_gt: bool
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--nodes",
        default="~/mowgli/mowgli_data/all/nodes_v002.csv",
        help="path to mowgli nodes.csv file",
    )
    parser.add_argument(
        "--edges",
        default="~/mowgli/mowgli_data/all/edges_v002.csv",
        help="path to mowgli edges.csv file",
    )
    parser.add_argument(
        "--output",
        default="~/mowgli/graph_data/all.graphml",
        help="output graphml file",
    )
    parser.add_argument(
        "--gt", action="store_true", help="whether to store to gt format too"
    )
    ...
    assert (
        ".graphml" in args.output
    ), f'output file must have ".graphml" format. > {args.output} '

    g = transform_to_graphtool_format(
        mowgli_edges_path=args.edges,
        mowgli_nodes_path=args.nodes,
        graphml_path=args.output,
        do_gt=args.gt,
    )
