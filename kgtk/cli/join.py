...
def add_arguments_extended(parser: KGTKArgumentParser, parsed_shared_args: Namespace):
    ...
    parser.add_argument(
        "left_file_path",
        help="The left-side KGTK file to join (required).",
        type=Path,
        default=None,
    )
    ...
    parser.add_argument(
        "right_file_path",
        help="The right-side KGTK file to join (required).",
        type=Path,
        default=None,
    )
    ...
    parser.add_argument(
        ...
        help="Left file join columns (default=None).",
        nargs="+",
    )
    ...
    parser.add_argument(
        ...
        help="Right file join columns (default=None).",
        nargs="+",
    )
    ...
    KgtkReaderOptions.add_arguments(
        parser, mode_options=True, who="left", expert=_expert, defaults=False
    )
    KgtkReaderOptions.add_arguments(
        parser, mode_options=True, who="right", expert=_expert, defaults=False
    )
    KgtkValueOptions.add_arguments(parser, expert=_expert)


def run(
    left_file_path: typing.Optional[Path],
    ...
    **kwargs  # Whatever KgtkFileOptions and KgtkValueOptions want.
    ...
    if (left_file_path is None or str(left_file_path) == "-") and (
        right_file_path is None or str(right_file_path) == "-"
    ):
        print(
            "The left and right files may not both be stdin.",
            file=error_file,
            flush=True,
        )
        return 1
    ...
    if show_options:
        ...
        print(
            "left: %s" % (str(left_file_path) if left_file_path is not None else "-"),
            file=error_file,
        )
        print(
            "right: %s" % (str(left_file_path) if left_file_path is not None else "-"),
            file=error_file,
        )
        print(
            "--output-file=%s"
            % (str(output_file_path) if output_file_path is not None else "-"),
            file=error_file,
        )
        ...
        if left_join_columns is not None:
            print(
                "--left-join-columns=%s" % " ".join(left_join_columns), file=error_file
            )
        if right_join_columns is not None:
            print(
                "--right-join-columns=%s" % " ".join(right_join_columns),
                file=error_file,
            )
