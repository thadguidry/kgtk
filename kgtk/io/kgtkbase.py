...
class KgtkBase(KgtkFormat):
    ...
    def _yelp(
        ...
        if error_action in [
            ValidationAction.REPORT,
            ValidationAction.COMPLAIN,
            ValidationAction.EXIT,
        ]:
            print(
                "In %s header '%s': %s" % (who, header_line, msg),
                file=error_file,
                flush=True,
            )
    ...
    def get_column_idx(
        ...
        for col_name in name_or_aliases:
            if col_name in column_name_map:
                if column_idx >= 0:
                    cls._yelp(
                        "Ambiguous required column names %s and %s"
        ...
        if column_idx < 0 and not is_optional:
            ...
            cls._yelp(
                "Missing required column: %s" % " | ".join(name_or_aliases),
                header_line=header_line,
                who=who,
                error_action=error_action,
                error_file=error_file,
    ...
    def check_column_name(
        ...
        if column_name.lstrip() != column_name:
            results.append(
                "Column name '%s' starts with leading white space" % column_name
            )
        if column_name.rstrip() != column_name:
            results.append(
                "Column name '%s' ends with leading white space" % column_name
            )
        if not (column_name.startswith('"') or column_name.startswith("'")):
            if "".join(column_name.split()) != column_name.strip():
                results.append(
                    "Column name '%s' contains internal white space" % column_name
                )
        if "," in column_name:
            results.append(
                "Warning: Column name '%s' contains a comma (,)" % column_name
            )
        if "|" in column_name:
            results.append(
                "Warning: Column name '%s' contains a vertical bar (|)" % column_name
            )
        # if ";" in column_name:
    ...
    def check_column_names(
        ...
        for column_name in column_names:
            gripes: typing.List[str] = cls.check_column_name(
                column_name, header_line, error_action, error_file
            )
    ...
    def build_column_name_map(
        ...
        for column_name in column_names:
            if column_name is None or len(column_name) == 0:
                cls._yelp(
                    "Column %d has an invalid name in the file header" % column_idx,
                    header_line=header_line,
                    who=who,
                    error_action=error_action,
                    error_file=error_file,
            ...
            if column_name in column_name_map:
                cls._yelp(
                    "Column %d (%s) is a duplicate of column %d"
                    % (column_idx, column_name, column_name_map[column_name]),
                    header_line=header_line,
                    who=who,
                    error_action=error_action,
                    error_file=error_file,
    ...
    def get_special_columns(
        ...
        node1_column_idx: int = cls.get_column_idx(
        ...
        label_column_idx: int = cls.get_column_idx(
            cls.LABEL_COLUMN_NAMES,
            column_name_map,
            header_line=header_line,
            who=who,
            error_action=error_action,
        ...
        node2_column_idx: int = cls.get_column_idx(
            cls.NODE2_COLUMN_NAMES,
            column_name_map,
            header_line=header_line,
            who=who,
            error_action=error_action,
        ...
        id_column_idx: int = cls.get_column_idx(
            cls.ID_COLUMN_NAMES,
            column_name_map,
            header_line=header_line,
            who=who,
    ...
    @classmethod
    def additional_edge_columns(
        cls, column_names: typing.List[str]
    ) -> typing.List[str]:
        """
        ...
        for column_name in column_names:
            if (
                column_name not in KgtkFormat.NODE1_COLUMN_NAMES
                and column_name not in KgtkFormat.NODE2_COLUMN_NAMES
                and column_name not in KgtkFormat.LABEL_COLUMN_NAMES
            ):
                additional_columns.append(column_name)
        return additional_columns

    @classmethod
    def additional_node_columns(
        cls, column_names: typing.List[str],
    ) -> typing.List[str]:
        """
        Return a list of column names in this file excluding the required columns.
