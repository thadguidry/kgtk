...
class KgtkWriter(KgtkBase):
    ...
    output_column_names: typing.List[str] = attr.ib(
        validator=attr.validators.deep_iterable(
            member_validator=attr.validators.instance_of(str),
            iterable_validator=attr.validators.instance_of(list),
        )
    ...
    gzip_thread: typing.Optional[GzipProcess] = attr.ib(
        validator=attr.validators.optional(attr.validators.instance_of(GzipProcess)),
        default=None,
    )
    ...
    output_format: str = attr.ib(
        validator=attr.validators.instance_of(str), default=OUTPUT_FORMAT_DEFAULT
    )  # TODO: use an enum
    ...
    def open(
        ...
        if verbose:
            print(
                "File_path.suffix: %s" % file_path.suffix, file=error_file, flush=True
            )
        ...
        if file_path.suffix in [".gz", ".bz2", ".xz", ".lz4"]:
            ...
            if file_path.suffix == ".gz":
                if verbose:
                    print(
                        "KgtkWriter: writing gzip %s" % str(file_path),
                        file=error_file,
                        flush=True,
                    )
                ...
                if verbose:
                    print(
                        "KgtkWriter: writing bz2 %s" % str(file_path),
                        file=error_file,
                        flush=True,
                    )
                ...
                if verbose:
                    print(
                        "KgtkWriter: writing lzma %s" % str(file_path),
                        file=error_file,
                        flush=True,
                    )
                ...
                if verbose:
                    print(
                        "KgtkWriter: writing lz4 %s" % str(file_path),
                        file=error_file,
                        flush=True,
                    )
                ...
                raise ValueError(
                    "Unexpected file_path.suffiz = '%s'" % file_path.suffix
                )
            ...
            if verbose:
                print(
                    "KgtkWriter: writing file %s" % str(file_path),
                    file=error_file,
                    flush=True,
                )
            return cls._setup(
    ...
    def _setup(
        ...
        if output_format is None:
            ...
            if verbose:
                print(
                    "Defaulting the output format to %s" % output_format,
                    file=error_file,
                    flush=True,
                )
        ...
        if output_column_names is None:
            ...
            if len(output_column_names) != len(column_names):
                raise ValueError(
                    "%s: %d column namess but %d output column namess"
                    % (who, len(column_names), len(output_column_names))
                )
        ...
        if old_column_names is not None or new_column_names is not None:
            ...
            if len(old_column_names) != len(new_column_names):
                raise ValueError(
                    "%s: old/new column name length mismatch: %d != %d"
                    % (who, len(old_column_names), len(new_column_names))
                )
            ...
            for idx, column_name in enumerate(old_column_names):
                if column_name not in output_column_names:
                    raise ValueError(
                        "%s: old column names %s not in the output column names."
                        % (who, column_name)
                    )
                output_column_names[
                    output_column_names.index(column_name)
                ] = new_column_names[idx]
        ...
        output_column_name_map: typing.Mapping[str, int] = cls.build_column_name_map(
            output_column_names,
            header_line=header,
            who=who,
            error_action=header_error_action,
    ...
    def json_map(
        self, values: typing.List[str], compact: bool = False
    ...
    def write_header(self):
        ...
        if self.output_format == "json":
            ...
            raise ValueError(
                "KgtkWriter: header: Unrecognized output format '%s'."
                % self.output_format
            )
    ...
    def write(
        ...
        if shuffle_list is not None:
            if len(shuffle_list) != len(values):
                # TODO: throw a better exception
                raise ValueError(
                    "The shuffle list is %d long but the values are %d long"
                    % (len(shuffle_list), len(values))
                )
        ...
        if self.require_all_columns and len(values) < self.column_count:
            ...
            raise ValueError(
                "Required %d columns in input line %d, saw %d: '%s'"
                % (self.column_count, self.line_count, len(values), line)
            )
        if self.prohibit_extra_columns and len(values) > self.column_count:
            line = self.column_separator.join(values)
            raise ValueError(
                "Required %d columns in input line %d, saw %d (%d extra): '%s'"
                % (
                    self.column_count,
                    self.line_count,
                    len(values),
        ...
        if self.output_format == "kgtk":
            ...
            self.writeline(
                json.dumps(self.json_map(values), indent=None, separators=(",", ":"))
                + ","
            )
        elif self.output_format == "json-map-compact":
            self.writeline(
                json.dumps(
                    self.json_map(values, compact=True),
                    indent=None,
                    separators=(",", ":"),
                )
                + ","
    ...
    def writemap(self, value_map: typing.Mapping[str, str]):
        ...
        if self.prohibit_extra_columns:
            for column_name in value_map.keys():
                if column_name not in self.column_name_map:
                    raise ValueError(
                        "Unexpected column name %s at data record %d"
                        % (column_name, self.line_count)
                    )
        ...
        for column_name in self.column_names:
            if column_name in value_map:
                ...
                # TODO: throw a better exception.
                raise ValueError(
                    "Missing column %s at data record %d"
                    % (column_name, self.line_count)
                )
            else:
                values.append("")
    ...
    def build_shuffle_list(
        self, other_column_names: typing.List[str], fail_on_unknown_column: bool = False
    ) -> typing.List[int]:
        ...
        for column_name in other_column_names:
            if column_name in self.column_name_map:
                ...
                # TODO: throw a better exception
                raise ValueError(
                    "Unknown column name %s when building shuffle list" % column_name
                )
            else:
...
def main():
    ...
    parser.add_argument(
        dest="input_kgtk_file", help="The KGTK file to read", type=Path, nargs="?"
    )
    parser.add_argument(
        dest="output_kgtk_file", help="The KGTK file to write", type=Path, nargs="?"
    )
    parser.add_argument(
        "--header-error-action",
        dest="header_error_action",
    ...
    parser.add_argument(
        "--gzip-in-parallel",
        dest="gzip_in_parallel",
        help="Execute gzip in a subthread.",
        action="store_true",
    )
    parser.add_argument(
        "--input-mode",
        dest="input_mode",
        help="Determine the input KGTK file mode.",
        type=KgtkReader.Mode,
        action=EnumNameAction,
        default=KgtkReader.Mode.AUTO,
    )
    parser.add_argument(
        "--output-mode",
        dest="output_mode",
        help="Determine the output KGTK file mode.",
        type=KgtkWriter.Mode,
        action=EnumNameAction,
        default=KgtkWriter.Mode.AUTO,
    )
    parser.add_argument(
        "--output-format",
        dest="output_format",
