            ...
            for each_line in f.readlines():
                ...
                if headers is None:
                    ...
                    if len(headers) < 2:
                        ...
                        column_references = {
                            "predicate": headers.index("predicate"),
                        ...
                        raise KGTKException(
                            "Can't determine which column is label column for label file!"
                        )
    ...
    for each_file in file_path:
        try:
            ...
            # zip file
                ...
                input_data = (
                    archive.read(each_file.replace(".zip", "")).decode().split("\n")
                )
...
def main(**kwargs):
    ...
    try:
        ...
        sentence_properties = {
            ...
            "description_properties": kwargs.get(
                "description_properties", ["description"]
            ),
        ...
        if property_labels_files:
            ...
            _logger.info(
                "Totally {} property labels loaded.".format(len(property_labels_dict))
            )
        else:
        ...
        for each_model_name in all_models_names:
            for each_input_file in input_uris:
                _logger.info(
                    "Running {} model on {}".format(each_model_name, each_input_file)
                )
                process = EmbeddingVector(
                    each_model_name,
...
def parser():
    return {"help": """Produce embedding vectors on given file's nodes."""}
...
def add_arguments(parser):
    ...
    parser.add_argument(
        ...
        default="bert-base-wikipedia-sections-mean-tokens",
