...
def parser():
    return {"help": "Import an wikidata file into KGTK file"}
...
def add_arguments(parser):
    ...
    """
    parser.add_argument(
        "-i", "--inp", action="store", type=str, dest="inp_path", help="input path file"
    )
    parser.add_argument(
        "--procs",
        action="store",
...
def run(
    inp_path, procs, node_file, edge_file, qual_file, limit, lang, source, deprecated
):
    ...
    class MyMapper(pyrallel.Mapper):
        ...
        def process(self, line, node_file, edge_file, qual_file, languages, doc_id):
                ...
                if edge_file or qual_file:
                    ...
                    if keep:
                        ...
                        for prop, claim_property in claims.items():
                            seq_no = 1
                            for cp in claim_property:
                                if (deprecated or cp["rank"] != "deprecated") and cp[
                                    "mainsnak"
                                ]["snaktype"] == "value":
                                    rank = cp["rank"]
                                    ...
                                    if typ.startswith("wikibase"):
                                        ...
                                        value = val["amount"]
                                        mag = val["amount"]
                                        if val.get("upperBound", None) or val.get(
                                            "lowerBound", None
                                        ):
                                        ...
                                        calendar = val.get("calendarmodel", "").split(
                                            "/"
                                        )[-1]
                                        value = (
                                            pre
                            ...
                            for link in sitelinks:
                                if link.endswith("wiki") and link != "commonswiki":
                                    sid = (
                                        qnode
                                        + "-wikipedia_sitelink-"
                                        + str(wikipedia_seq_no)
                                    )
                                    wikipedia_seq_no += 1
                                    sitetitle = "_".join(
                                        sitelinks[link]["title"].split()
                                    )
            ...
            if node_file:
                with open(
                    node_file + "_{}".format(self._idx), write_mode, newline=""
                ) as myfile:
                    for row in nrows:
                        wr = csv.writer(
                            myfile,
            ...
            if edge_file:
                with open(
                    edge_file + "_{}".format(self._idx), write_mode, newline=""
                ) as myfile:
                    for row in erows:
                        wr = csv.writer(
                            myfile,
            ...
            if qual_file:
                with open(
                    qual_file + "_{}".format(self._idx), write_mode, newline=""
                ) as myfile:
                    for row in qrows:
                        wr = csv.writer(
                            myfile,
