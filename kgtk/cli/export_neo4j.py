...
class Node:
    ...
    def serialize(self, node_id):
        ...
        if self.instance_type:
            serialized_node = "CREATE ({}:{} {})".format(
                node_id, serialized_instance_type, serialized_properties
            )
        else:
...
class Relationship:
    ...
    def serialize(self):
        if self.properties:
            ...
            serialized_properties = ""
        serialized_relationship = "({})-[:{} {}]->({}),".format(
            self.src_node, self.name, serialized_properties, self.dst_node
        )
...
def create_graph(
    ...
    if statement_file_name:
        with open(
            statement_file_name, "r", encoding=statement_file_encoding
        ) as input_file:
            ...
            for statement in statements:
                ...
                if is_item(statement["node2"]):
                    if statement["property"] == "P31":
                        ...
                        dst_node = node_store.get_or_create(
                            clean_label(statement["node2"])
                        )
                        if "node2_label" in statement and statement["node2_label"]:
                            dst_node.add_property("label", statement["node2_label"])
                        relationship = relationship_store.get_or_create(
                            clean_label(statement["id"])
                        )
                        relationship.add_names(
                            clean_label(statement["property"]),
                            clean_label(statement["node1"]),
                            clean_label(statement["node2"]),
                        )
                        if (
                            "property_label" in statement
                            and statement["property_label"]
                        ):
                            relationship.add_property(
                                "label", statement["property_label"]
                            )
                else:
                    src_node.add_property(
                        clean_label(statement["property"]), statement["node2"]
                    )
    ...
    if qualifier_file_name:
        with open(
            qualifier_file_name, "r", encoding=qualifier_file_encoding
        ) as input_file:
            ...
            for qualifier in qualifiers:
                ...
                if relationship:
                    relationship.add_property(
                        clean_label(qualifier["property"]), qualifier["node2"]
                    )
    return graph
...
def add_arguments(parser):
    ...
    parser.add_argument(
        ...
        action="store",
        type=str,
        dest="statement_file_path",
        help="Filepath of the statement file",
        default="",
    )
    parser.add_argument(
        "-qf",
        "--qualifier_file_path",
        type=str,
        dest="qualifier_file_path",
        help="Filepath of the qualifier file",
        default="",
    )
    parser.add_argument(
        "-o",
        ...
        help="Directory where the result file will be saved",
        default="",
    )
    parser.add_argument(
        "-se",
        "--statement_file_encoding",
        type=str,
        dest="statement_file_encoding",
        help="Encoding of the statement file, eg.: UTF-8",
        default="",
    )
    parser.add_argument(
        "-qe",
        "--qualifier_file_encoding",
        type=str,
        dest="qualifier_file_encoding",
        help="Encoding of the qualifier file, eg.: UTF-8",
        default="",
    )
